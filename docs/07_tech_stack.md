...
| データ保存 | **ブラウザ内保存(localStorage)** | GitHub Pages だけで動かすため、サーバー・DBを持たない構成に変更(2026-07-29)。各ユーザーのデータは端末内のみに保存。バックアップの書き出し/読み込み機能つき。※サーバー版(SQLite/Turso対応)は git 履歴に保存済みで、ログイン・共有機能が必要になったら復帰可能 |
